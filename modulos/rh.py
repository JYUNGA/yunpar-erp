...
def calcular_horas_netas(marcas_str, es_sabado, es_domingo):
    ...
    return (
        round(horas_totales, 2),
        round(horas_extras, 2),
        round(horas_descuento, 2),
        es_falta,
    )
...
def render(supabase):
    ...
    if "rh_version" not in st.session_state:
        st.session_state["rh_version"] = 1
    if "df_master_rh" not in st.session_state:
        st.session_state["df_master_rh"] = pd.DataFrame()
    if "periodo_cargado" not in st.session_state:
        st.session_state["periodo_cargado"] = None
    if "mes_anio_txt" not in st.session_state:
    ...
    tab_cfg, tab_auditoria, tab_pago = st.tabs(
        [
            "⚙️ Configuración Personal",
            "📥 Subir Biométrico (Auditoría Interactiva)",
            "💸 Liquidación y Pagos",
        ]
    )
    ...
    with tab_cfg:
        ...
        with col1:
            ...
            with st.form("form_rh_emp", clear_on_submit=True):
                ...
                bio_id = st.number_input(
                    "ID Biométrico (Código Reloj)", min_value=1, step=1
                )
                    ...
                    if nom:
                        try:
                            supabase.table("rh_empleados").insert(
                                {
                                    "nombre_completo": nom,
                                    "biometrico_id": int(bio_id),
                                    "sueldo_base": float(s_base),
                                }
                            ).execute()
                            ...
                            st.error(f"Error: {e}")
        with col2:
            st.subheader("Parámetros del Personal")
            st.caption(
                "💡 Doble clic para corregir. Desmarca '¿Activo?' si alguien renunció."
            )

            # 1. Traemos a TODOS los empleados (activos e inactivos)
            emps = supabase.table("rh_empleados").select("*").order("id").execute().data
            if emps:
                ...
                df_editor = st.data_editor(
                    ...
                    column_config={
                        "id": None,
                        "biometrico_id": st.column_config.NumberColumn(
                            "ID Reloj", step=1
                        ),
                        "nombre_completo": st.column_config.TextColumn(
                            "Nombre en Sistema"
                        ),
                        "sueldo_base": st.column_config.NumberColumn(
                            "Sueldo Base ($)", format="%.2f"
                        ),
    ...
    with tab_auditoria:
        ...
        archivo_bio = st.file_uploader(
            ...
            type=["csv", "xls", "xlsx"],
            key=f"bio_uploader_{st.session_state['uploader_key']}",
        )
        ...
        if archivo_bio:
            try:
                ...
                if st.session_state["periodo_cargado"] != nombre_archivo:
                    with st.spinner("Analizando estructura de asistencia..."):
                        ...
                        if nombre_archivo.endswith(".xls") or nombre_archivo.endswith(
                            ".xlsx"
                        ):
                            excel_file = pd.ExcelFile(archivo_bio)
                            hoja_usar = (
                                "Attendance Record"
                                if "Attendance Record" in excel_file.sheet_names
                                else excel_file.sheet_names[0]
                            )
                            df_inspect = pd.read_excel(
                                archivo_bio, sheet_name=hoja_usar, nrows=15, header=None
                            )
                            header_idx = -1
                            for idx, row in df_inspect.iterrows():
                                if "Employee ID" in str(row.values) and "Name" in str(
                                    row.values
                                ):
                                    header_idx = idx
                                    break
                            for row in df_inspect.values:
                                for cell in row:
                                    if isinstance(cell, str) and "Made Date:" in cell:
                                        match = re.search(
                                            r"Made Date:(\d{4}/\d{2})", cell
                                        )
                                        if match:
                                            mes_anio_txt = match.group(1).replace(
                                                "/", "-"
                                            )
                            df_raw = (
                                pd.read_excel(
                                ...
                                if header_idx != -1
                                else pd.read_excel(archivo_bio, sheet_name=hoja_usar)
                            )
                            df_raw.columns = df_raw.columns.astype(str).str.strip()
                        else:
                            try:
                                contenido = archivo_bio.getvalue().decode("utf-8")
                            except UnicodeDecodeError:
                                contenido = archivo_bio.getvalue().decode("latin-1")
                            if contenido:
                                ...
                                header_idx = next(
                                    (
                                        ...
                                        if "Employee ID" in l and "Name" in l
                                    ),
                                    -1,
                                )
                                if header_idx != -1:
                        ...
                        if (
                            df_raw.empty
                            or "Employee ID" not in df_raw.columns
                            or "Name" not in df_raw.columns
                        ):
                            st.error(
                                "🚨 La estructura de este archivo no coincide con las columnas esperadas."
                            )
                            st.stop()

                        cols_dias = [
                            str(i) for i in range(1, 32) if str(i) in df_raw.columns
                        ]
                        df_melt = df_raw.melt(
                            id_vars=["Employee ID", "Name"],
                        ...
                        df_melt = df_melt[
                            pd.to_numeric(
                                df_melt["Employee ID"], errors="coerce"
                            ).notnull()
                        ]

                        # Solo auditamos a los que tienen la casilla 'activo' marcada
                        map_emps = (
                            {
                                e["biometrico_id"]: e["nombre_completo"]
                                for e in emps
                        ...
                        for _, row in df_melt.iterrows():
                            ...
                            if bio_id_file not in map_emps:
                                continue
                            ...
                            meta_diaria = (
                                0 if es_domingo else (4.5 if es_sabado else 8.5)
                            )

                            # Analizar si tiene marcas o no
                            marcas_str = (
                                str(row["Marcas"]).strip()
                                if pd.notna(row["Marcas"])
                                else ""
                            )
                            ...
                            if marcas_str == "" or marcas_str.lower() == "nan":
                                ...
                                if es_domingo:
                                    continue  # La lógica de domingos con marca la procesaremos si es estrictamente necesario, por ahora saltamos

                                h_tot, h_ext, h_des, es_falta = calcular_horas_netas(
                                    row["Marcas"], es_sabado, es_domingo
                                )
                ...
                if df_master.empty:
                    ...
                    if st.button(
                        "🔄 APLICAR CAMBIOS Y RECALCULAR PANEL",
                        type="secondary",
                        use_container_width=True,
                    ):
                        with st.spinner("Procesando fórmulas matemáticas..."):
                            for emp in empleados_unicos:
                                editor_key = (
                                    f"editor_rh_{emp}_{st.session_state['rh_version']}"
                                )
                                if editor_key in st.session_state:
                                    cambios = st.session_state[editor_key].get(
                                        "edited_rows", {}
                                    )
                                    df_emp_indices = df_master[
                                        df_master["Empleado"] == emp
                                    ].index.tolist()
                                    ...
                                    for local_idx_str, columnas in cambios.items():
                                        ...
                                        if "Horas Trabajadas" in columnas:
                                            nuevo_valor = float(
                                                columnas["Horas Trabajadas"]
                                            )
                                            df_master.at[
                                                global_idx, "Horas Trabajadas"
                                            ] = nuevo_valor

                                            meta = float(
                                                df_master.at[
                                                    global_idx, "Horas Requeridas"
                                                ]
                                            )
                                            extras = max(0.0, nuevo_valor - meta)
                                            atrasos = max(0.0, meta - nuevo_valor)

                                            df_master.at[global_idx, "Horas Extras"] = (
                                                round(extras, 2)
                                            )
                                            df_master.at[global_idx, "Horas Atraso"] = (
                                                round(atrasos, 2)
                                            )
                                            df_master.at[
                                                global_idx, "¿Falta Injustificada?"
                                            ] = (nuevo_valor == 0.0)

                            st.session_state["df_master_rh"] = df_master
                            st.session_state["rh_version"] += 1
                            st.success(
                                "¡Operación completada! Todas las alertas y métricas se han actualizado."
                            )
                            time.sleep(0.5)
                            st.rerun()
                    ...
                    for emp in empleados_unicos:
                        df_emp_especifico = (
                            df_master[df_master["Empleado"] == emp]
                            .copy()
                            .reset_index(drop=True)
                        )
                        num_alertas = len(
                            df_emp_especifico[
                                df_emp_especifico["Horas Trabajadas"] == 0
                            ]
                        )
                        label_expander = (
                            f"👤 {emp} — ({num_alertas} Alertas/Marcas incompletas)"
                        )
                        ...
                        with st.expander(label_expander, expanded=(num_alertas > 0)):
                            ...
                            df_editado = st.data_editor(
                                ...
                                column_config={
                                    ...
                                    "Día de la Semana": st.column_config.TextColumn(
                                        "Día/Fecha", disabled=True
                                    ),
                                    "Marcaciones": st.column_config.TextColumn(
                                        "Marcas Reloj (Incompletas)", disabled=True
                                    ),
                                    "Horas Requeridas": st.column_config.NumberColumn(
                                        "Debe Trabajar", format="%.2f", disabled=True
                                    ),
                                    "Horas Trabajadas": st.column_config.NumberColumn(
                                        "Horas Reales (Edita aquí)",
                                        format="%.2f",
                                        min_value=0.0,
                                        max_value=24.0,
                                    ),
                                    "Horas Extras": st.column_config.NumberColumn(
                                        "Extras", format="%.2f", disabled=True
                                    ),
                                    "Horas Atraso": st.column_config.NumberColumn(
                                        "Atrasos", format="%.2f", disabled=True
                                    ),
                                    "¿Falta Injustificada?": st.column_config.CheckboxColumn(
                                        "Falta (Desmarcar si justificada)"
                                    ),
                                },
                                hide_index=True,
                                use_container_width=True,
                            ...
                            c_m2.metric(
                                "Horas Calculadas Fichero", f"{total_calc:.2f} hrs"
                            )
                            c_m3.metric(
                                "Horas Extras (Actualizado)",
                                f"{total_ext:.2f} hrs",
                                delta_color="normal",
                            )
                            c_m4.metric(
                                "Horas Atraso (Actualizado)",
                                f"{total_atr:.2f} hrs",
                                delta_color="inverse",
                            )
                    ...
                    st.write("---")
                    if st.button(
                        "💾 GUARDAR ASISTENCIA AUDITADA EN BASE DE DATOS",
                        type="primary",
                        use_container_width=True,
                    ):
                        with st.spinner("Guardando datos depurados..."):
                            map_ids_internos = {
                                e["nombre_completo"]: e["id"] for e in emps
                            }

                            # DICCIONARIO PARA DEDUPLICAR LA CARGA
                            payload_temp = {}

                            for _, r in st.session_state["df_master_rh"].iterrows():
                                emp_id_interno = map_ids_internos.get(r["Empleado"])
                                if emp_id_interno:
                                    # Usamos la combinación (ID, Fecha) como llave única.
                                    # Si hay un duplicado, el último valor sobreescribe al anterior, evitando el error de BD.
                                    llave = (int(emp_id_interno), str(r["Fecha_BD"]))
                            ...
                            # Convertimos el diccionario purificado en una lista para Supabase
                            payload_final = list(payload_temp.values())

                            try:
                                supabase.table("rh_asistencia").upsert(
                                    payload_final, on_conflict="empleado_id, fecha"
                                ).execute()
                                ...
                                st.success(
                                    "¡Todo el archivo mensual ha sido guardado y archivado con éxito! El panel se ha limpiado."
                                )
                                time.sleep(1.2)
                                st.rerun()
                            except Exception as e:
            ...
            st.info(
                "A la espera de un fichero del reloj biométrico para iniciar la auditoría."
            )
    ...
    with tab_pago:
        ...
        mes_pago = st.selectbox(
            "Seleccione el Mes con datos a Liquidar:", meses_disponibles
        )

        if st.button(
            "🔄 Calcular Nómina del Mes Seleccionado", use_container_width=True
        ):
            try:
                ...
                res_asist = (
                    supabase.table("rh_asistencia")
                    .select(
                        "*, observaciones, rh_empleados(nombre_completo, sueldo_base)"
                    )
            ...
            if res_asist:
                ...
                df_asist["Nombre"] = df_asist["rh_empleados"].apply(
                    lambda x: x["nombre_completo"]
                )
                df_asist["Sueldo Base"] = df_asist["rh_empleados"].apply(
                    lambda x: float(x["sueldo_base"])
                )

                resumen = (
                    df_asist.groupby(["empleado_id", "Nombre", "Sueldo Base"])
                    .agg(
                        {
                ...
                resumen["Valor Hora"] = round(
                    resumen["Sueldo Base"] / 240.0, 4
                )  # 4 decimales para exactitud

                # 2. Cálculo de Descuentos por faltas/atrasos (Horas no trabajadas de la jornada exigida)
                resumen["Descuentos ($)"] = round(
                    resumen["horas_descuento"] * resumen["Valor Hora"], 2
                )

                # 3. Pago de Horas Extras (a x1 según lo acordado)
                resumen["Bono Extras ($)"] = round(
                    resumen["horas_extras"] * resumen["Valor Hora"], 2
                )

                # 4. Neto a Pagar: Sueldo completo - lo que faltó + lo que se quedó tiempo extra
                resumen["Neto a Pagar ($)"] = round(
                    resumen["Sueldo Base"]
                    - resumen["Descuentos ($)"]
                    + resumen["Bono Extras ($)"],
                    2,
                ...
                df_mostrar.columns = [
                    ...
                    "Hrs Extras",
                    "Bono Extras (+ $)",
                    "Neto Total a Recibir ($)",
                ]

                st.markdown(
                    f"### 📋 Reporte de Pagos para el periodo: {traducir_periodo(mes_pago)}"
                )
                st.caption(
                    "ℹ️ *Se asume el sueldo mensual íntegro, se descuentan los atrasos/faltas, y se suman las horas extras.*"
                )
                ...
                st.write("")
                try:
                    zip_bytes = generar_zip_roles(
                        resumen, df_asist, traducir_periodo(mes_pago)
                    )
                    st.download_button(
                ...
                st.write("---")
                st.markdown("#### 🏦 Parámetros de Pago Contable")
                col_met, col_ban = st.columns(2)
                metodo_pago_nomina = col_met.selectbox(
                    "Método de Pago", ["Transferencia", "Efectivo", "Cheque"]
                )

                banco_origen = None
                if metodo_pago_nomina != "Efectivo":
                    banco_origen = col_ban.selectbox(
                        "Banco Origen",
                        [
                            "Seleccionar...",
                            "JEP",
                ...
                if st.button(
                    "💰 Liquidar Mes y Enviar a Finanzas",
                    type="primary",
                    use_container_width=True,
                ):
                    # Validación estricta para evitar pagos fantasma
                    if (
                        metodo_pago_nomina != "Efectivo"
                        and banco_origen == "Seleccionar..."
                    ):
                        st.error(
                            "⚠️ Para transferencias o cheques, debes seleccionar obligatoriamente de qué banco sale el dinero."
                        )
                    else:
                        with st.spinner(
                            "Registrando egresos individuales y enlazando contabilidad..."
                        ):
                            for _, row in resumen.iterrows():
                                # 1. Insertamos la liquidación en RRHH y capturamos su ID
                                res_liq = (
                                    supabase.table("rh_liquidaciones")
                                    .insert(
                                        {
                                ...
                                id_liq = res_liq.data[0]["id"]

                                # 2. Insertamos el egreso en Finanzas con trazabilidad total
                                supabase.table("egresos").insert(
                                    {
                                        ...
                                        "metodo_pago": metodo_pago_nomina,
                                        "banco": (
                                            banco_origen
                                        ...
                                        "referencia_id": id_liq,
                                    }
                                ).execute()

                            st.success(
                                f"¡Éxito! La nómina de {mes_pago} ha sido procesada de manera correcta y enlazada a Finanzas."
                            )
            else:
                st.info(f"No hay registros archivados para {mes_pago}.")
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        for _, row in resumen_df.iterrows():
            ...
            pdf.cell(
                0,
                6,
                f"Comprobante Mensual de Liquidacion | Periodo: {mes_pago}",
                ln=True,
                align="R",
            )
            pdf.ln(12)
            ...
            pdf.cell(
                120,
                8,
                f"Descuento por Atrasos/Faltas ({row['horas_descuento']:.2f} hrs):",
            )
            ...
            pdf.cell(
                120,
                8,
                f"Bono por Horas Extras Acumuladas ({row['horas_extras']:.2f} hrs):",
            )
            ...
            # --- ANEXO DIARIO ---
            df_emp_diario = df_diario[df_diario["empleado_id"] == emp_id].sort_values(
                by="fecha"
            )
            if not df_emp_diario.empty:
                ...
                pdf.cell(
                    0,
                    8,
                    "ANEXO COMPLETO: Registro Diario de Asistencia y Tiempos",
                    ln=True,
                )
                ...
                for _, d_row in df_emp_diario.iterrows():
                    ...
                    if (
                        marcas_reales == "None"
                        or marcas_reales.strip() == ""
                        or marcas_reales == "nan"
                    ):
                    ...
                    pdf.cell(
                        34,
                        6,
                        f"{d_row['horas_trabajadas']:.2f} hrs",
                        border=1,
                        align="C",
                    )
                    pdf.cell(
                        34, 6, f"{d_row['horas_extras']:.2f} hrs", border=1, align="C"
                    )
                    pdf.cell(
                        34,
                        6,
                        f"{d_row['horas_descuento']:.2f} hrs",
                        border=1,
                        align="C",
                        ln=True,
                    )
