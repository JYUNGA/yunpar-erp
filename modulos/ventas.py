    ...
    def header(self):
        ...
        if self.datos_cabecera:
            ...
            self.cell(
                90,
                5,
                limpiar_texto_pdf(self.datos_cabecera.get("cliente_nombre", "")),
                0,
                0,
                "L",
            )
...
def generar_pdf_venta(datos_venta):
    ...
    cabecera = {
        ...
        "cliente_nombre": cli.get(
            "nombre_completo", cli.get("nombre", "Consumidor Final")
        ),
    ...
    for item in datos_venta.get("items", []):
        ...
        pdf.line(
            52, pdf.get_y(), 52 + w_cod + w_desc + w_cant + w_unit + w_tot, pdf.get_y()
        )
    ...
    try:
        total_letras = num2words(total, lang="es").upper() + " DÓLARES"
    except:
        total_letras = f"{total} DÓLARES"
    ...
    pdf.cell(25, 5, f"${subt_calc:.2f}", 0, 1, "R")
    pdf.set_x(x_totales)
    pdf.cell(20, 5, "IVA (15%):", 0, 0, "R")
    pdf.cell(25, 5, f"${iva_calc:.2f}", 0, 1, "R")
    pdf.set_x(x_totales)
    pdf.set_font("Arial", "B", 11)
    pdf.set_text_color(18, 38, 122)
    pdf.cell(20, 6, "TOTAL:", 0, 0, "R")
    pdf.cell(25, 6, f"${total:.2f}", 0, 1, "R")

    pdf.set_x(x_totales)
    pdf.set_font("Arial", "", 10)
    pdf.set_text_color(0, 100, 0)
    pdf.cell(20, 5, "Abono:", 0, 0, "R")
    pdf.cell(25, 5, f"${abono:.2f}", 0, 1, "R")
    pdf.set_x(x_totales)
    pdf.set_text_color(200, 0, 0)
    pdf.cell(20, 5, "Saldo:", 0, 0, "R")
    pdf.cell(25, 5, f"${saldo:.2f}", 0, 1, "R")
    ...
    try:
        qr.add_data(qr_data_string.encode("utf-8").decode("latin-1"))
    except:
        qr.add_data("Datos Venta")
    qr.make(fit=True)
    ...
    pdf.set_xy(55, pos_firma)
    pdf.cell(50, 0, "_______________________", 0, 1, "C")
    pdf.set_xy(55, pos_firma)
    pdf.cell(50, 5, "Dpto. Ventas / Caja", 0, 0, "C")
    pdf.set_xy(120, pos_firma)
    pdf.cell(50, 0, "_______________________", 0, 1, "C")
    pdf.set_xy(120, pos_firma)
    pdf.cell(50, 5, "Cliente Conforme", 0, 1, "C")
    pdf.set_xy(120, pos_firma + 4)
...
# ==========================================
def render(supabase):
    if "rol" not in st.session_state or st.session_state["rol"] not in [
        "GERENTE",
        "VENDEDORA",
    ]:
    ...
    with tab1:
        ...
        with col_busqueda:
            ...
            fecha_venta_seleccionada = col_fec1.date_input(
                "🗓️ Fecha de Registro (Venta y Pago)", value=obtener_fecha_actual()
            )
            fecha_entrega_seleccionada = col_fec2.date_input(
                "📅 Fecha de Entrega Estimada", value=obtener_fecha_actual()
            )
            ...
            with st.container(border=True):
                ...
                if st.session_state.get("vd_cliente_id"):
                    found = next(
                        (
                            ...
                            if v == st.session_state["vd_cliente_id"]
                        ),
                        None,
                    )
                ...
                sel_cli = c1.selectbox(
                    "Cliente",
                    ["Consumidor Final"] + list(mapa_cli.keys()),
                    index=idx_sel,
                ...
                with c2.popover("➕ Crear Cliente Nuevo", use_container_width=True):
                    with st.form("vd_nc_full", clear_on_submit=True):
                        ...
                        f_tip = st.selectbox("Tipo", opc_tipo, key="vd_new_cli_tip")
                        f_gen = st.selectbox(
                            "Género",
                        ...
                        if st.form_submit_button("Guardar Cliente"):
                            if f_ruc and f_nom:
                                ...
                                if res_c.data:
                                    st.session_state["vd_cliente_id"] = res_c.data[0][
                                        "id"
                                    ]
            ...
            with st.expander("🔍 Filtros de Búsqueda (Catálogo)", expanded=True):
                ...
                if not df_p.empty:
                    ...
                    eda = cf3.selectbox(
                        "Edad",
                        ["Todos"]
                        + sorted(list(df_p["grupo_edad_limpio"].dropna().unique())),
                    )

                    txt_p = st.text_input(
                        "Buscar texto...", placeholder="Cód o Nombre de producto"
                    )
                    ...
                    if eda != "Todos":
                        df_fin = df_fin[df_fin["grupo_edad_limpio"] == eda]
                    if txt_p:
                        df_fin = df_fin[
                            df_fin["descripcion"].str.contains(txt_p, case=False)
                            | df_fin["codigo_referencia"].str.contains(
                                txt_p, case=False
                            )
                        ]
                    ...
                    sel_p_key = st.selectbox(
                        "Seleccione el producto:", list(mapa_p.keys())
                    )
                    prod_obj = mapa_p.get(sel_p_key, None)
                else:
                    st.warning(
                        "⚠️ No se encontraron productos clasificados estrictamente para VENTA en la Base de Datos."
                    )
                    prod_obj = None
            ...
            if prod_obj:
                ...
                if es_impresion:
                    st.info(
                        "🖨️ **Servicio de Impresión.** Configura los archivos para calcular el cobro."
                    )

                    try:
                        res_telas_bd = (
                            supabase.table("insumos").select("nombre").execute()
                        )
                        ...
                        lista_telas_db = (
                            [opcion_nula] + [t["nombre"] for t in res_telas_bd.data]
                            if res_telas_bd.data
                            else [opcion_nula, "Estándar"]
                        )
                    except:
                    ...
                    st.info(
                        "💡 **Tip:** Límite 200MB. Para archivos más pesados, usa el script local y sube aquí solo el archivo Excel/CSV."
                    )

                    archivos = st.file_uploader(
                        "Arrastra aquí los archivos:",
                        type=["pdf", "xlsx", "csv"],
                        accept_multiple_files=True,
                        key=st.session_state["uploader_key_vd"],
                    )

                    if st.button(
                        "📥 Procesar Archivos Subidos", use_container_width=True
                    ):
                        if archivos:
                            peso_total_mb = sum([f.size for f in archivos]) / (
                                1024 * 1024
                            )

                            if peso_total_mb > 200.0:
                                st.error(
                                    f"🛑 **¡ALERTA DE SOBRECARGA!** Peso total: {peso_total_mb:.1f} MB. Máximo permitido: 200 MB."
                                )
                            else:
                                for archivo in archivos:
                                    ...
                                    if nombre_archivo.endswith(".pdf"):
                                        ...
                                        st.session_state[
                                            "temp_archivos_impresion"
                                        ].append(
                                            {
                                                "Nombre": nom,
                                                "Perfil": "Plotter 1",
                                                "Tela": lista_telas_db[0],
                                                "Ancho (m)": anc,
                                                "Largo (m)": lar,
                                                "Cantidad": 1,
                                                "Notas": "",
                                    ...
                                    elif nombre_archivo.endswith(
                                        ".csv"
                                    ) or nombre_archivo.endswith(".xlsx"):
                                        try:
                                            df_local = (
                                                pd.read_csv(archivo)
                                ...
                                st.session_state["uploader_key_vd"] = str(
                                    datetime.now().timestamp()
                                )
                                st.rerun()
                        else:
                            st.warning(
                                "⚠️ No has seleccionado ningún archivo para procesar."
                            )
                    ...
                    with st.expander("➕ 2. Cargar datos de archivo manualmente"):
                        with st.form("form_manual_ventas", clear_on_submit=True):
                            ...
                            a_in = col_m3.number_input(
                                "Ancho (m)", min_value=0.0, step=0.01
                            )
                            l_in = col_m4.number_input(
                                "Largo (m)", min_value=0.0, step=0.01
                            )
                            c_in = col_m5.number_input(
                                "Cant", min_value=1, step=1, value=1
                            )
                    ...
                    if not df_archivos_vd.empty:
                        ...
                        edited_archivos = st.data_editor(
                            ...
                            hide_index=True,
                            key=f"editor_vd_{prod_obj['id']}",
                        )

                        if st.button(
                            "🔄 Borrar Seleccionados y Actualizar",
                            use_container_width=True,
                        ):
                            df_kept = (
                                edited_archivos[~edited_archivos["Eliminar"]]
                                .copy()
                        ...
                        largo_total_calculado = (
                            edited_archivos["Largo (m)"] * edited_archivos["Cantidad"]
                        ).sum()
                    else:
                        st.info("No hay archivos en la lista.")
                        largo_total_calculado = 0.0

                    cantidad_cobro = st.number_input(
                        "Total Metros a Cobrar",
                        value=float(largo_total_calculado),
                        min_value=0.0,
                        step=0.1,
                    )
                else:
                    ...
                    cantidad_cobro = st.number_input(
                        "Cantidad", min_value=0.01, value=1.0, step=0.1, format="%.2f"
                    )
                ...
                if st.button("➕ Agregar al Carrito", type="primary"):
                    if es_impresion and not edited_archivos.empty:
                        df_final = (
                            edited_archivos[~edited_archivos["Eliminar"]]
                            if "Eliminar" in edited_archivos.columns
                            else edited_archivos
                        )
        ...
        with col_resumen:
            ...
            if not st.session_state["carrito_vd"]:
                ...
                for i, item in enumerate(st.session_state["carrito_vd"]):
                    ...
                    if item["archivos"]:
                        st.caption(
                            f"📎 {len(item['archivos'])} archivos listos para plotter."
                        )
                    st.divider()
                ...
                with st.container(border=True):
                    ...
                    if modalidad_pago == "Pago Total (100%)":
                        abono = st.number_input(
                            "Monto Recibido ($)",
                            value=float(total_venta),
                            disabled=True,
                        )
                    elif modalidad_pago == "Crédito / Sin Abono ($0)":
                        abono = st.number_input(
                            "Monto Recibido ($)", value=0.0, disabled=True
                        )
                    else:
                        abono = st.number_input(
                            "Monto Recibido ($)",
                    ...
                    if st.button(
                        ...
                        if (
                            ...
                            and banco == "Seleccionar..."
                        ):
                            st.error(
                                "⚠️ Debes seleccionar a qué banco ingresó el dinero."
                            )
                            st.stop()
                        ...
                        try:
                            with st.spinner("Registrando venta y enviando archivos..."):
                                id_creador = st.session_state.get("id_usuario", None)
                                data_orden = {
                                ...
                                # Registrar la creación en el historial inmediatamente después del INSERT
                                transicionar_estado(
                                    supabase,
                                    id_orden,
                                    estado_orden,
                                    id_creador,
                                    notas="Creación por Venta Directa (VD)",
                                )
                                    ...
                                    supabase.table("items_orden").insert(
                                        {
                                            ...
                                            "cantidad_total": item[
                                                "cantidad"
                                            ],  # Guardamos la cantidad cobrable real
                                            "familia_producto": (
                                                "IMPRESION"
                                                if item["es_impresion"]
                                                else "GENERICO"
                                            ),
                                        }
                                    ).execute()
                                            ...
                                            payloads_plotter.append(
                                                {
                                                    ...
                                                    "perfil_color": arch.get(
                                                        "perfil", "Plotter 1"
                                                    ),
                                                    "tela": arch.get(
                                                        "tela", "Estándar"
                                                    ),
                                                    "notas_disenador": arch.get(
                                                        "notas", ""
                                                    ),
                                                }
                                            )
    ...
    with tab2:
        ...
        f_desde = col_f1.date_input(
            "Desde", value=obtener_fecha_actual(), key="vd_f_des"
        )
        f_hasta = col_f2.date_input(
            "Hasta", value=obtener_fecha_actual(), key="vd_f_has"
        )
        txt_bus = col_bus.text_input(
            "🔍 Buscar Venta", placeholder="Código VD o Nombre..."
        )
        ...
        try:
            ...
            query = (
                supabase.table("ordenes")
                .select(
                    "id, codigo_orden, total_estimado, abono_inicial, saldo_pendiente, estado, clientes(nombre_completo)"
                )
                .ilike("codigo_orden", "VD-%")
            )
            query = query.gte("created_at", f"{f_desde}T00:00:00").lte(
                "created_at", f"{f_hasta}T23:59:59"
            )
            ...
            if res_hist.data:
                ...
                for d in res_hist.data:
                    ...
                    if (
                        txt_bus
                        and txt_bus.lower() not in d["codigo_orden"].lower()
                        and txt_bus.lower() not in nom_cli.lower()
                    ):
                        continue
                            ...
                            with c_info:
                                ...
                                if pagos:
                                    for p in pagos:
                                        ...
                                        st.caption(
                                            f"- ${float(p['monto']):.2f} recibidos en {banco} (Ref: {ref})"
                                        )
                                else:
                                    st.caption(
                                        "- Venta a Crédito / Sin abono inicial registrado."
                                    )
                                ...
                                if st.button(
                                    ...
                                    try:
                                        ...
                                        if id_v:
                                            ...
                                            supabase.table(
                                                "archivos_impresion"
                                            ).delete().eq("orden_id", id_v).execute()
                                            ...
                                            st.success(
                                                f"Venta {cod_sel} eliminada correctamente de finanzas y caja."
                                            )
                                            time.sleep(1.5)
                                            st.rerun()
                                        else:
                                            st.error(
                                                "No se localizó el ID único de la transacción."
                                            )
                                    except Exception as e:
                                        st.error(
                                            f"Error crítico al eliminar venta: {e}"
                                        )
                else:
                    st.info(
                        "No hay ventas que coincidan con tu búsqueda en este rango de fechas."
                    )
            else:
