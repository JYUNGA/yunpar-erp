...
def render(supabase):
    ...
    with tab1:
        ...
        txt_bus_pend = col_p3.text_input(
            "🔍 Buscar",
            placeholder="Orden, RUC o Nombre de Cliente...",
            key="pend_txt_bus",
        )
        ...
        try:
            ...
            # --- FILTRO DE AUDITORÍA FINANCIERA (ELIMINADO) ---
            # Mostramos TODAS las órdenes sin facturar para que el personal
            # pueda auditarlas y corregir pagos faltantes si es necesario.
            # if datos_combinados:
            ...
            if datos_combinados:
                ...
                # 1. Inicialización segura de la variable
                evt_pend = None
                ...
                if not df_pend.empty:
                    ...
                    st.info("Ninguna orden coincide con los criterios de búsqueda.")

                # 2. Verificación de existencia antes de leer atributos
                if evt_pend is not None and len(evt_pend.selection.rows) > 0:
                    ...
                    if datos_orden:
                        ...
                        with col_cli:
                            ...
                            st.write("RUC / CI:")
                            st.code(
                                cli.get("cedula_ruc", "9999999999999"), language=None
                            )

                            st.write("Correo:")
                            st.code(
                                cli.get(
                                    "email", cli.get("correo", "Sin correo registrado")
                                ),
                                language=None,
                            )
                            ...
                            st.code(
                                cli.get(
                                    "telefono",
                                    cli.get("celular", "Sin teléfono registrado"),
                                ),
                                language=None,
                            )
                            ...
                            st.write("Dirección:")
                            st.code(
                                cli.get("ciudad", "Sin dirección registrada"),
                                language=None,
                            )
                        ...
                        st.caption(
                            "Verifica que los cobros registrados coincidan con la realidad. Modifica, agrega o elimina pagos si el asesor cometió un error al registrar el abono/saldo."
                        )
                        ...
                        with col_val1:
                            if descuadre > 0:
                                st.error(
                                    f"⚠️ Faltan pagos por registrar: **${descuadre:.2f}** pendientes de cobro."
                                )
                            elif descuadre < 0:
                                st.warning(
                                ...
                                st.success(
                                    "✅ Los pagos cubren el 100% de la orden exactamente."
                                )
                        ...
                        st.caption(
                            "Ingresa aquí el número de factura para limpiar esta orden de la bandeja. (El botón se bloquea si hay deudas)."
                        )
                        ...
                        if col_emit2.button(
                            ...
                            if num_factura.strip():
                                try:
                                    ...
                                    st.success(
                                        f"¡Listo! Orden {cod_sel} vinculada a la factura {num_factura}."
                                    )
                                    time.sleep(1.5)
                                    st.rerun()
                                except Exception as e:
