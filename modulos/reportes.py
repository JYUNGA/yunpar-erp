    ...
    for item in items:
        ...
        if key not in agrupados:
            agrupados[key] = {
                ...
                "especificaciones_producto": list(
                    item.get("especificaciones_producto", [])
                ),
            }
        else:
            agrupados[key]["cantidad_total"] += float(item.get("cantidad_total", 0.0))
            agrupados[key]["especificaciones_producto"].extend(
                item.get("especificaciones_producto", [])
            )
...
def obtener_ultimas_ordenes(supabase_client):
    try:
        res = (
            supabase_client.table("ordenes")
            .select("codigo_orden, estado, fecha_entrega, total_estimado")
    ...
    try:
        ...
        if orden_data.get("creado_por_id"):
            res_creador = (
                ...
                .eq("id", orden_data["creado_por_id"])
                .execute()
            )
            orden_data["creador"] = (
                res_creador.data[0].get("nombre_completo", "Desconocido")
                if res_creador.data
                else "Desconocido"
            )
        else:
            orden_data["creador"] = "No registrado"

        res_items = (
            supabase_client.table("items_orden")
            .select("*")
        ...
        if not items:
            try:
                res_det = (
                    supabase_client.table("detalles_orden")
                    .select("*")
                ...
                if res_det.data:
                    for d in res_det.data:
                        ...
                        d["id"] = d.get(
                            "id_detalle", 0
                        )  # <-- FIX: Traducimos id_detalle a id para evitar el error
                    items = res_det.data
            except:
                pass
        ...
        for item in items:
            if item.get("producto_id"):
                try:
                    ...
                    res_prod = (
                        supabase_client.table("productos_catalogo")
                        .select("descripcion, tipo_prenda")
            ...
            if id_tela:
                try:
                    res_tela = (
                        supabase_client.table("insumos")
                        .select("nombre")
                        .eq("id", id_tela)
                        .execute()
                    )
                    item["nombre_tela"] = (
                        res_tela.data[0]["nombre"] if res_tela.data else "Estándar"
                    )
                except:
                    item["nombre_tela"] = "Estándar"
            else:
                item["nombre_tela"] = "Estándar"

            res_esp = (
                supabase_client.table("especificaciones_producto")
                .select("*")
                .eq("item_orden_id", item["id"])
                .execute()
            )
...
def buscar_lista_ordenes(supabase_client, codigo="", cliente="", fechas=None):
    try:
        ...
        query = supabase_client.table("ordenes").select(
            "codigo_orden, estado, fecha_entrega, total_estimado, cliente_id, created_at, saldo_pendiente"
        )
        if codigo:
            query = query.ilike("codigo_orden", f"%{codigo.strip()}%")
        if fechas and len(fechas) == 2:
            inicio = fechas[0].strftime("%Y-%m-%d")
            fin = fechas[1].strftime("%Y-%m-%d")
            query = query.gte("created_at", f"{inicio}T00:00:00").lte(
                "created_at", f"{fin}T23:59:59"
            )
        ...
        if cliente_ids:
            ...
            for c in res_cli.data:
                mapa_clientes[c["id"]] = c.get(
                    "nombre_completo", c.get("nombre", "Desconocido")
                )
        ...
        for d in data:
            ...
            if termino_cli and termino_cli not in nom_cli.lower():
                continue
            lista_limpia.append(
                {
                    ...
                    "Saldo_Num": float(
                        d.get("saldo_pendiente", 0)
                    ),  # Campo oculto que usaremos para la lógica
                }
            )
        return lista_limpia
    except:
...
class PDFComprobante(FPDF):
    def header(self):
        try:
            self.image("FONDOYUNPAR.png", x=0, y=0, w=210, h=297)
        except:
            pass
        self.set_y(50)
    ...
    def footer(self):
        ...
        self.cell(
            0,
            10,
            f"Impreso el: {fecha_imp}  |  Página {self.page_no()}/{{nb}}",
            align="L",
        )
...
class PDFTaller(FPDF):
    ...
    def footer(self):
        ...
        self.cell(
            0,
            10,
            f"Documento Interno de Producción  |  Página {self.page_no()}/{{nb}}",
            align="C",
        )
        self.set_text_color(0, 0, 0)
...
def generar_comprobante_cliente(orden):
    ...
    pdf.cell(
        0, 12, f"{orden['codigo_orden']}", new_x="LMARGIN", new_y="NEXT", align="L"
    )
    ...
    nombre_cliente = limpiar_texto_pdf(
        cli.get("nombre_completo", cli.get("nombre", "Consumidor Final"))
    )
    telefono = limpiar_texto_pdf(
        cli.get("telefono", cli.get("celular", "No registrado"))
    )
    ...
    ancho_etiqueta1 = 27
    ancho_valor1 = 75
    ancho_etiqueta2 = 31  # Aumentamos 5mm al ancho_valor1 para separar la columna

    pdf.cell(ancho_etiqueta1, 6, "Cliente:", border=False)
    pdf.set_font("helvetica", "", 10)
    pdf.cell(ancho_valor1, 6, f"{nombre_cliente}", border=False)
    pdf.set_font("helvetica", "B", 10)
    pdf.cell(ancho_etiqueta2, 6, "Fecha Pedido:", border=False)
    pdf.set_font("helvetica", "", 10)
    pdf.cell(
        0,
        6,
        f"{formatear_fecha_es(orden.get('created_at'))}",
        border=False,
        new_x="LMARGIN",
        new_y="NEXT",
    )
    ...
    pdf.set_font("helvetica", "B", 10)
    pdf.cell(ancho_etiqueta1, 6, "Teléfono:", border=False)
    pdf.set_font("helvetica", "", 10)
    pdf.cell(ancho_valor1, 6, f"{telefono}", border=False)
    pdf.set_font("helvetica", "B", 10)
    pdf.cell(ancho_etiqueta2, 6, "Fecha Entrega:", border=False)
    pdf.set_font("helvetica", "", 10)
    pdf.cell(
        0,
        6,
        f"{formatear_fecha_es(orden.get('fecha_entrega'))}",
        border=False,
        new_x="LMARGIN",
        new_y="NEXT",
    )
    ...
    pdf.set_font("helvetica", "B", 10)
    pdf.cell(ancho_etiqueta1, 6, "Correo:", border=False)
    pdf.set_font("helvetica", "", 10)
    pdf.cell(ancho_valor1, 6, f"{correo}", border=False)
    pdf.set_font("helvetica", "B", 10)
    pdf.cell(ancho_etiqueta2, 6, "Diseñador:", border=False)
    pdf.set_font("helvetica", "", 10)
    pdf.cell(0, 6, f"{disenador}", border=False, new_x="LMARGIN", new_y="NEXT")

    # Fila 4: Atendido por
    pdf.set_font("helvetica", "B", 10)
    pdf.cell(ancho_etiqueta1, 6, "Atendido por:", border=False)
    pdf.set_font("helvetica", "", 10)
    pdf.cell(0, 6, f"{creador}", border=False, new_x="LMARGIN", new_y="NEXT")
    pdf.ln(6)

    pdf.set_font("helvetica", "", 10)
    with pdf.table(
        col_widths=(115, 20, 25, 30), text_align=("LEFT", "CENTER", "RIGHT", "RIGHT")
    ) as table:
        estilo_cabecera = FontFace(
            fill_color=(0, 51, 153), color=(255, 255, 255), emphasis="B"
        )
        row = table.row(style=estilo_cabecera)
        for header in ["Producto", "Cantidad", "Precio Unit.", "Subtotal"]:
            row.cell(header)
        estilo_datos = FontFace(
            fill_color=(255, 255, 255), color=(0, 0, 0), emphasis=""
        )

        for item in items_financieros:
            row = table.row(style=estilo_datos)
            nombre_prod = limpiar_texto_pdf(
                str(item.get("nombre_producto", "Producto no definido"))
                .replace("│", "|")
                .replace("—", "-")
            )
            row.cell(nombre_prod)
            row.cell(str(item.get("cantidad_total", 0)))
            ...
            if precio <= 0:
                ...
                row.cell(f"${precio:.2f}")
                row.cell(f"${item.get('cantidad_total', 0) * precio:.2f}")
    ...
    pdf.ln(2)
    pdf.set_font("helvetica", "B", 10)  # Letra más pequeña y menos salto
    x_offset = 150
    pdf.set_x(x_offset)
    pdf.cell(25, 5, "Total:", align="R")
    pdf.cell(20, 5, f"${total_orden:.2f}", align="R", new_x="LMARGIN", new_y="NEXT")

    # 2. Imprimimos la suma real de todos los abonos en lugar del estático
    pdf.set_x(x_offset)
    pdf.cell(25, 5, "Total Abonado:", align="R")
    pdf.cell(20, 5, f"${abono_real:.2f}", align="R", new_x="LMARGIN", new_y="NEXT")

    pdf.set_x(x_offset)
    pdf.cell(25, 5, "Saldo:", align="R")
    pdf.set_text_color(200, 0, 0)
    pdf.cell(20, 5, f"${saldo_orden:.2f}", align="R", new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(0, 0, 0)
    ...
    if url_imagen and pagos:
        ...
        pdf.cell(
            0,
            8,
            "Historial de Pagos y Notas Generales:",
            align="L",
            new_x="LMARGIN",
            new_y="NEXT",
        )
        ...
        estilo_cabecera_pagos = FontFace(
            fill_color=(0, 51, 153), color=(255, 255, 255), emphasis="B"
        )
        estilo_datos_pagos = FontFace(
            fill_color=(255, 255, 255), color=(0, 0, 0), emphasis=""
        )
        ...
        with pdf.table(
            ...
            for h in ["Fecha", "Banco/Método", "Monto"]:
                row.cell(h)
            for p in pagos:
                row = t_pagos.row(style=estilo_datos_pagos)
                f_pago = p.get("fecha_pago", "")
                if f_pago and len(f_pago) >= 10:
                    f_pago = f"{f_pago[8:10]}/{f_pago[5:7]}/{f_pago[2:4]}"
                banco = limpiar_texto_pdf(
                    p.get("banco_destino") or p.get("metodo_pago") or "Efectivo"
                )
                row.cell(f_pago)
                row.cell(str(banco)[:20])
                row.cell(f"${float(p.get('monto', 0)):.2f}")
        ...
        pdf.set_font("helvetica", "", 8)
        observaciones = limpiar_texto_pdf(
            str(orden.get("observaciones_generales") or "Ninguna").strip()
        )
        ...
        pdf.set_font("helvetica", "B", 10)
        pdf.cell(
            0, 8, "Referencia de Diseño:", align="L", new_x="LMARGIN", new_y="NEXT"
        )
        ...
        if espacio_restante >= 70:
            ...
            try:
                ...
                pdf.cell(0, 10, "(Imagen no disponible)", align="C")
            necesita_nueva_hoja_anexo = (
                True  # La hoja 1 se llenó con la imagen, el anexo va a la hoja 2
            )
        else:
            ...
            try:
                ...
                pass
            necesita_nueva_hoja_anexo = (
                False  # El anexo va en esta MISMA hoja 2, debajo de la imagen
            )

    elif url_imagen and not pagos:
        pdf.set_font("helvetica", "B", 10)
        pdf.cell(
            0, 10, "Referencia de Diseño:", align="L", new_x="LMARGIN", new_y="NEXT"
        )
        ...
        with pdf.table(
            col_widths=(40, 80, 40), text_align=("CENTER", "LEFT", "RIGHT")
        ) as t_pagos:
            row = t_pagos.row(style=estilo_cabecera_pagos)
            for h in ["Fecha", "Banco/Método", "Monto"]:
                row.cell(h)
            for p in pagos:
                ...
                f_pago = p.get("fecha_pago", "")
                if f_pago and len(f_pago) >= 10:
                    f_pago = f"{f_pago[8:10]}/{f_pago[5:7]}/{f_pago[0:4]}"
                banco = limpiar_texto_pdf(
                    p.get("banco_destino") or p.get("metodo_pago") or "Efectivo"
                )
                row.cell(f_pago)
    ...
    if necesita_nueva_hoja_anexo:
        ...
        pdf.ln(
            5
        )  # Un pequeño margen de separación visual si comparte hoja con la imagen
    ...
    pdf.cell(
        0,
        10,
        "ANEXO: DETALLE TÉCNICO APROBADO",
        align="C",
        new_x="LMARGIN",
        new_y="NEXT",
    )
    ...
    pdf.multi_cell(
        0,
        5,
        "El presente anexo detalla las especificaciones exactas (tallas, nombres, números y materiales) solicitadas para la orden. Por favor, verifique que la información detallada a continuación sea correcta.",
    )
    pdf.ln(5)

    estilo_cabecera_anexo = FontFace(
        fill_color=(0, 51, 153), color=(255, 255, 255), emphasis="B"
    )
    estilo_datos_anexo = FontFace(
        fill_color=(255, 255, 255), color=(0, 0, 0), emphasis=""
    )

    for item in items_anexo_taller:
        nombre_prod = limpiar_texto_pdf(
            str(item.get("nombre_producto", "Producto"))
            .replace("│", "|")
            .replace("—", "-")
        )
        ...
        for esp in especificaciones_crudas:
            key = (
                ...
                bool(
                    esp.get("es_arquero", False)
                ),  # Añadimos el booleano para no mezclar arqueros con jugadores
            )
        ...
        if familia in ["UNIFORME COMPLETO", "PRENDA SUPERIOR"]:
            especificaciones.sort(
                key=lambda x: (
                    orden_talla(x.get("talla_superior")),
                    orden_talla(x.get("talla_inferior")),
                )
            )
        ...
        if not especificaciones:
            ...
            pdf.cell(
                0,
                8,
                "  Sin lista de detalles para este producto.",
                border=1,
                new_x="LMARGIN",
                new_y="NEXT",
            )
        ...
        pdf.set_font("helvetica", "", 9)
        tiene_polines = (
            any(bool(esp.get("talla_polines")) for esp in especificaciones)
        ...
        if familia == "UNIFORME COMPLETO" and tiene_polines:
            ...
            cols = (15, 25, 30, 120)
            headers = ["Cant", "Talla Inf.", "Num", "Observación"]
        else:
            cols = (15, 25, 150)
            headers = ["Cant", "Talla", "Observaciones"]

        with pdf.table(
            col_widths=cols,
            text_align=(
                ("CENTER", "CENTER", "LEFT", "CENTER", "LEFT", "CENTER", "LEFT")
                if tiene_polines
                else ("CENTER", "CENTER", "LEFT", "CENTER", "LEFT", "LEFT")
            ),
        ) as t_anexo:
            ...
            for esp in especificaciones:
                ...
                cant = str(
                    esp.get("cant_fila", 1)
                )  # <--- REEMPLAZAMOS EL "1" FIJO POR EL VALOR CALCULADO DINÁMICAMENTE
                ...
                cuello = limpiar_texto_pdf(
                    str(esp.get("tipo_cuello_texto") or "").strip()
                )
                obs = limpiar_texto_pdf(
                    str(esp.get("observacion_individual") or "").strip()
                )
    ...
    pdf.multi_cell(
        0,
        5,
        "DECLARACIÓN DE APROBACIÓN: Revisé detalladamente y apruebo que los nombres, números, tallas, cuellos, telas y observaciones detalladas en este anexo son correctas para el inicio de la producción.",
    )
    pdf.ln(25)
    start_firma_y = pdf.get_y()

    # Dibujo Cliente
    pdf.set_y(start_firma_y - 8)
    pdf.set_x(25)
    pdf.set_font("times", "I", 14)
    pdf.set_text_color(0, 0, 100)  # Simula firma en azul
    pdf.cell(60, 5, f"{nombre_cliente}", align="C")
    pdf.set_text_color(0, 0, 0)  # Restaurar color
    pdf.set_y(start_firma_y)
    pdf.set_x(25)
    pdf.line(25, start_firma_y, 85, start_firma_y)
    pdf.set_x(25)
    pdf.set_font("helvetica", "B", 9)
    pdf.cell(60, 5, "Firma del Cliente", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.set_x(25)
    pdf.set_font("helvetica", "", 8)
    pdf.cell(60, 5, f"C.I: ________________", align="C")
    ...
    pdf.set_y(start_firma_y)
    pdf.set_x(125)
    pdf.line(125, start_firma_y, 185, start_firma_y)
    pdf.set_x(125)
    pdf.set_font("helvetica", "B", 9)
    pdf.cell(
        60, 5, "Firma del Asesor / Vendedor", align="C", new_x="LMARGIN", new_y="NEXT"
    )
...
def generar_hoja_produccion(orden):
    ...
    pdf.cell(
        100, 12, f"ORDEN: {limpiar_texto_pdf(orden['codigo_orden'])}", border=False
    )
    ...
    pdf.cell(
        0,
        12,
        limpiar_texto_pdf(f"ENTREGA: {formatear_fecha_es(orden.get('fecha_entrega'))}"),
        align="R",
        new_x="LMARGIN",
        new_y="NEXT",
    )
    ...
    pdf.cell(
        0,
        6,
        limpiar_texto_pdf(f"Diseñador: {disenador}  |  Asesor: {creador}"),
        new_x="LMARGIN",
        new_y="NEXT",
    )
    pdf.cell(
        0,
        6,
        limpiar_texto_pdf(f"Cliente: {nombre_cliente}"),
        new_x="LMARGIN",
        new_y="NEXT",
    )
    ...
    if orden.get("alerta_cambios"):
        ...
        pdf.cell(
            0,
            10,
            " ¡ATENCIÓN: ESTA ORDEN HA TENIDO CAMBIOS RECIENTES!",
            fill=True,
            align="C",
            new_x="LMARGIN",
            new_y="NEXT",
    ...
    for item in items_taller:
        ...
        nombre_crudo = (
            str(item.get("nombre_producto") or fam).strip().upper().replace("│", "|")
        )
        ...
        if len(partes) > 1:
            ...
            if not any(
                x in p1 for x in ["TX", "XL", "ADULTO", "NIÑ", "TALLA", "SRTA"]
            ) and not (p1.startswith("T") and p1[1:2].isdigit()):
                titulo_base += f" {p1}"
        ...
        for esp in item.get("especificaciones_producto", []):
            ...
            if tiene_sup:
                tit_s = (
                    f"{titulo_base} (SUPERIOR)"
                    if (tiene_inf or fam == "UNIFORME COMPLETO")
                    else titulo_base
                )
                if es_arq:
                    tit_s += " (ARQ)"
                if tit_s not in resumenes_dinamicos:
                    resumenes_dinamicos[tit_s] = {}
                resumenes_dinamicos[tit_s][t_sup] = (
                    resumenes_dinamicos[tit_s].get(t_sup, 0) + 1
                )

            # Conteo Inferior Universal (Eliminamos el hardcoding de familias)
            if tiene_inf:
                # Excepciones estrictas de tu requerimiento anterior
                if "FALDA SHORT" in nombre_crudo:
                    tit_i = "FALDA SHORT"
                elif "SHORT" in nombre_crudo:
                    tit_i = "SHORT"
                elif "FALDA" in nombre_crudo:
                    tit_i = "FALDA"
                else:
                    tit_i = (
                        f"{titulo_base} (INFERIOR)"
                        if (tiene_sup or fam == "UNIFORME COMPLETO")
                        else titulo_base
                    )

                if es_arq:
                    tit_i += " (ARQ)"
                if tit_i not in resumenes_dinamicos:
                    resumenes_dinamicos[tit_i] = {}
                resumenes_dinamicos[tit_i][t_inf] = (
                    resumenes_dinamicos[tit_i].get(t_inf, 0) + 1
                )
    ...
    pdf.cell(
        0,
        8,
        "REFERENCIA VISUAL DE CORTE Y CONFECCIÓN",
        align="C",
        new_x="LMARGIN",
        new_y="NEXT",
    )
    ...
    if resumenes_dinamicos or resumen_polines:
        pdf.set_font("helvetica", "B", 12)
        pdf.cell(
            0, 8, "RESUMEN GLOBAL DE CORTE", align="C", new_x="LMARGIN", new_y="NEXT"
        )
        pdf.ln(2)
        ...
        for titulo, tallas_dict in resumenes_dinamicos.items():
            if tallas_dict:
                tablas_a_dibujar.append(
                    {"titulo": titulo, "datos": tallas_dict, "tipo": "normal"}
                )
        if resumen_polines:
            tablas_a_dibujar.append(
                {"titulo": "POLINES", "datos": resumen_polines, "tipo": "polin"}
            )
        ...
        for tabla in tablas_a_dibujar:
            ...
            if col_actual >= 4:
                ...
                y_actual = (
                    max_y + 8
                )  # Bajamos un poco respecto al final de la tabla más larga
                pdf.set_y(y_actual)
            ...
            if "ARQ" in tit_upper:
                ...
                "(SUPERIOR)" in tit_upper
                or "(INFERIOR)" in tit_upper
                or "UNIF" in tit_upper
            ):
            ...
            pdf.set_text_color(
                0, 0, 0
            )  # Texto negro para que contraste con los tonos pastel
            ...
            if tabla["tipo"] == "normal":
                datos_ord = sorted(
                    tabla["datos"].items(), key=lambda x: orden_talla(x[0])
                )
                ...
                datos_ord = sorted(
                    tabla["datos"].items(),
                    key=lambda x: (orden_talla(x[0][0]), x[0][1]),
                )
            ...
            pdf.set_fill_color(
                r_c, g_c, b_c
            )  # Reutilizamos el mismo color de la cabecera
    ...
    estilo_cabecera_taller = FontFace(
        fill_color=(50, 50, 50), color=(255, 255, 255), emphasis="B"
    )
    estilo_datos_taller = FontFace(
        fill_color=(255, 255, 255), color=(0, 0, 0), emphasis=""
    )

    # NUEVO ESTILO: Resaltado amarillo soft para arqueros (fill amarillo, texto negro)
    estilo_datos_arquero = FontFace(
        fill_color=(255, 255, 102), color=(0, 0, 0), emphasis=""
    )

    for item in items_taller:
        familia = item.get("familia_producto", "GENERICO").upper()
        nombre_prod = limpiar_texto_pdf(
            str(item.get("nombre_producto", familia))
            .replace("│", "|")
            .replace("—", "-")
        )
        especificaciones_crudas = item.get("especificaciones_producto", [])
        ...
        for esp in especificaciones_crudas:
            key = (
                ...
                str(
                    esp.get("genero") or ""
                ).strip(),  # [Seguro]: Nueva variable de aislamiento
                str(esp.get("nombre_jugador") or "").strip(),
                str(esp.get("numero_dorsal") or "").strip(),
                str(esp.get("tipo_cuello_texto") or "").strip(),
                str(esp.get("talla_polines") or "").strip(),
        ...
        titulo_prod = limpiar_texto_pdf(
            f" PRODUCTO: {nombre_prod}   |   CANTIDAD TOTAL: {item.get('cantidad_total', 0)}\n TELA A USAR: {tela}"
        )
        ...
        with pdf.table(
            col_widths=cols, text_align=aligns, cell_fill_mode="ROWS"
        ) as table:
            ...
            for esp in especificaciones:
                ...
                obs_limpia = (
                    str(esp.get("observacion_individual") or "").strip().upper()
                )
                ...
                nom_limpio = limpiar_texto_pdf(
                    str(esp.get("nombre_jugador") or "").strip()
                )
                num_limpio = limpiar_texto_pdf(
                    str(esp.get("numero_dorsal") or "").strip()
                )
                cuello_limpio = limpiar_texto_pdf(
                    str(esp.get("tipo_cuello_texto") or "").strip()
                )
                obs_limpia_txt = limpiar_texto_pdf(
                    str(esp.get("observacion_individual") or "").strip()
                )
                ...
                if familia == "UNIFORME COMPLETO":
                    ...
                    row.cell(
                        limpiar_texto_pdf(
                            str(esp.get("talla_superior") or "-").strip() or "-"
                        )
                    )
                    row.cell(
                        limpiar_texto_pdf(
                            str(esp.get("talla_inferior") or "-").strip() or "-"
                        )
                    )
                    row.cell(gen_limpio)
                    row.cell(nom_limpio)
                    ...
                    if tiene_polines:
                        row.cell(
                            limpiar_texto_pdf(
                                str(esp.get("talla_polines") or "-").strip() or "-"
                            )
                        )
                    ...
                    row.cell(limpiar_texto_pdf(str(esp.get("acabado") or "").strip()))
                    row.cell(obs_limpia_txt)
                else:
                    row.cell(c_fila)
                    row.cell(limpiar_texto_pdf(str(esp.get("acabado") or "").strip()))
    ...
    observaciones = limpiar_texto_pdf(
        str(orden.get("observaciones_generales") or "").strip()
    )
    if observaciones and observaciones.lower() not in ["none", "null", "", "ninguna"]:
        ...
        pdf.set_fill_color(
            255, 230, 150
        )  # Un fondo amarillo cálido para llamar la atención
        pdf.cell(
            0,
            8,
            " NOTAS Y OBSERVACIONES GENERALES PARA PRODUCCIÓN",
            border=1,
            fill=True,
            align="L",
            new_x="LMARGIN",
        ...
        pdf.multi_cell(
            0, 8, limpiar_texto_pdf(f" {observaciones}"), border=1, align="L"
        )
        pdf.ln(5)
...
def extraer_datos_etiquetas(ordenes):
    ...
    def es_talla_valida(t):
        ...
        if t_str in ["", "-", "NONE", "NULL", "N/A", "NAN", "0"]:
            return False
        return True
...
def generar_etiquetas_pdf(datos_etiquetas):
    ...
    for i, data in enumerate(datos_etiquetas):
        ...
        if data["talla"]:
            pdf.text(
                x=base_x + OFFSET_X_TALLA,
                y=base_y + OFFSET_Y_TALLA,
                txt=limpiar_texto_pdf(data["talla"]),
            )
        if data["numero"]:
            pdf.text(
                x=base_x + OFFSET_X_NUMERO,
                y=base_y + OFFSET_Y_NUMERO,
                txt=limpiar_texto_pdf(data["numero"]),
            )
        if data["nombre"]:
            pdf.text(
                x=base_x + OFFSET_X_NOMBRE,
                y=base_y + OFFSET_Y_NOMBRE,
                txt=limpiar_texto_pdf(data["nombre"]),
            )
        ...
        if data.get("codigo"):
            ...
            pdf.set_font(
                "helvetica", "B", 11
            )  # Un punto menos para que encaje elegante en el nuevo ancho
...
def render_modulo_reportes(supabase_client):
    ...
    with st.expander("🔍 Buscador Avanzado", expanded=True):
        with st.form("buscar_orden_form"):
            ...
            busqueda_cli = col2.text_input(
                "Parte del nombre del Cliente", placeholder="Ej: Fra"
            )
            busqueda_fechas = col3.date_input(
                "Rango de Fechas (Pedido)", value=[], format="DD/MM/YYYY"
            )

            st.write("")
            submit_search = col4.form_submit_button(
                "Filtrar Repositorio", use_container_width=True
            )

    if "lista_ordenes" not in st.session_state or submit_search:
        with st.spinner("Cargando repositorio de órdenes..."):
            if submit_search:
                st.session_state.lista_ordenes = buscar_lista_ordenes(
                    supabase_client, busqueda_cod, busqueda_cli, busqueda_fechas
                )
            else:
    ...
    if st.session_state.lista_ordenes:
        ...
        st.caption(
            "📌 **Navega por las pestañas y selecciona cualquier orden para ver su detalle y generar PDFs:**"
        )
        ...
        if actual_evt1 != st.session_state.ultima_seleccion["evt1"]:
            codigos_seleccionados = (
                df_nuevas.iloc[actual_evt1]["Código"].tolist()
                if len(actual_evt1) > 0
                else []
            )
            st.session_state.ultima_seleccion["evt1"] = actual_evt1
        elif actual_evt2 != st.session_state.ultima_seleccion["evt2"]:
            codigos_seleccionados = (
                df_proceso.iloc[actual_evt2]["Código"].tolist()
                if len(actual_evt2) > 0
                else []
            )
            st.session_state.ultima_seleccion["evt2"] = actual_evt2
        elif actual_evt3 != st.session_state.ultima_seleccion["evt3"]:
            codigos_seleccionados = (
                df_pagadas.iloc[actual_evt3]["Código"].tolist()
                if len(actual_evt3) > 0
                else []
            )
            st.session_state.ultima_seleccion["evt3"] = actual_evt3
        else:
            if len(actual_evt1) > 0:
                codigos_seleccionados = df_nuevas.iloc[actual_evt1]["Código"].tolist()
            elif len(actual_evt2) > 0:
                codigos_seleccionados = df_proceso.iloc[actual_evt2]["Código"].tolist()
            elif len(actual_evt3) > 0:
                codigos_seleccionados = df_pagadas.iloc[actual_evt3]["Código"].tolist()

        if codigos_seleccionados:
            codigos_en_memoria = [
                o.get("codigo_orden") for o in st.session_state.ordenes_actuales
            ]
        ...
        st.info(
            "No hay órdenes registradas o no se encontraron coincidencias con los filtros."
        )
    ...
    with espacio_detalle.container():
        if st.session_state.ordenes_actuales:
            ...
            datos_etiquetas, resumen_ordenes = extraer_datos_etiquetas(
                st.session_state.ordenes_actuales
            )
            ...
            if len(st.session_state.ordenes_actuales) == 1:
                ...
                with col_info:
                    ...
                    nombre_cliente = cliente_data.get(
                        "nombre_completo",
                        cliente_data.get("nombre", "Cliente sin nombre registrado"),
                    )
                ...
                with st.expander(
                    "🔎 Auditoría Rápida (Productos y Trazabilidad de Pagos)",
                    expanded=False,
                ):
                    ...
                    with c_prod:
                        ...
                        for it in orden.get("items", []):
                            ...
                            cant_str = (
                                int(cant) if cant.is_integer() else f"{cant:.2f}m"
                            )
                            st.caption(
                                f"- {cant_str}x {it.get('nombre_producto', 'Producto')} | Subtotal: ${cant*precio:.2f}"
                            )
                    ...
                    with c_pag:
                        ...
                        if pagos:
                            for p in pagos:
                                ...
                                if f_pago and len(f_pago) >= 10:
                                    f_pago = (
                                        f"{f_pago[8:10]}/{f_pago[5:7]}/{f_pago[0:4]}"
                                    )
                                st.caption(
                                    f"- ${float(p['monto']):.2f} en {banco} (Ref: {ref}) el {f_pago}"
                                )
                        else:
                ...
                if total_etiq > 0:
                    st.info(
                        f"🏷️ **Resumen de Etiquetas:** {total_etiq} etiquetas ocuparán {hojas} hoja(s) | ⬜ {vacios} espacios vacíos al final."
                    )
                    ...
                    if st.session_state.get("rol") != "IMPRESION":
                        if st.button(
                            ...
                            with st.spinner("Generando..."):
                                st.session_state[f"pdf_comp_{cod}"] = (
                                    generar_comprobante_cliente(orden)
                                )
                        if f"pdf_comp_{cod}" in st.session_state:
                            st.download_button(
                                label="⬇️ Descargar Comprobante",
                                data=st.session_state[f"pdf_comp_{cod}"],
                                file_name=f"Comprobante_{cod}.pdf",
                ...
                with col_pdf2:
                    if st.button(
                        ...
                        with st.spinner("Generando..."):
                            st.session_state[f"pdf_prod_{cod}"] = (
                                generar_hoja_produccion(orden)
                            )
                    if f"pdf_prod_{cod}" in st.session_state:
                        st.download_button(
                            label="⬇️ Descargar Producción",
                            data=st.session_state[f"pdf_prod_{cod}"],
                            file_name=f"Produccion_{cod}.pdf",
                            mime="application/pdf",
                            width="stretch",
                            key=f"dl_prod_{cod}",
                        )
                ...
                with col_pdf3:
                    if total_etiq > 0:
                        ...
                        st.download_button(
                            ...
                            data=pdf_etiq,
                            file_name=f"Etiquetas_{cod}.pdf",
                            mime="application/pdf",
                            type="secondary",
                            width="stretch",
                            key=f"dl_etiq_{cod}",
                ...
                st.subheader(
                    f"📦 Modo Lote: {len(st.session_state.ordenes_actuales)} órdenes seleccionadas"
                )
                ...
                if total_etiq > 0:
                    ...
                    with col_res1:
                        st.success(
                            f"**Total a Imprimir:** {total_etiq} etiquetas ocuparán {hojas} hoja(s).\n\n**Espacios vacíos al final (retazos salvados):** {vacios}"
                        )
                        desglose = " | ".join(
                            [f"{cod}: {cant}" for cod, cant in resumen_ordenes.items()]
                        )
                    ...
                    st.warning(
                        "⚠️ Ninguna de las órdenes seleccionadas contiene uniformes válidos para etiquetar."
                    )
