...
def render(supabase):
    ...
    st.markdown(
        "Agrupa los polines de diferentes órdenes, normaliza los colores escritos por las vendedoras y marca la compra como realizada."
    )
    ...
    with st.spinner("Cargando requerimientos de polines..."):
        ...
        mapa_usu = (
            {u["id"]: u["nombre_completo"] for u in res_usu.data}
            if res_usu.data
            else {}
        )
        ...
        res_ord = (
            ...
            .select(
                "id, codigo_orden, estado, saldo_pendiente, creado_por_id, clientes(nombre_completo)"
            )
        ...
        df_ord["estado_limpio"] = (
            df_ord["estado"].fillna("").astype(str).str.strip().str.upper()
        )
        ...
        if df_ord.empty:
            st.success(
                "Todas las órdenes recientes ya fueron finalizadas, entregadas o están pagadas al 100%."
            )
            return

        df_ord["Vendedora"] = df_ord["creado_por_id"].map(
            lambda x: mapa_usu.get(x, "Desconocido")
        )
        df_ord["Cliente"] = df_ord["clientes"].apply(
            lambda x: x.get("nombre_completo", "S/N") if isinstance(x, dict) else "S/N"
        )
        ...
        for i in range(0, len(ids_ordenes_pendientes), tamanio_lote):
            ...
            res_lote = (
                ...
                .select("id, orden_id, familia_producto")
                .in_("familia_producto", ["UNIFORME COMPLETO", "POLIN"])
                .in_("orden_id", lote_ids)
                .execute()
            )
            if res_lote.data:
                items_data.extend(res_lote.data)
        ...
        for i in range(0, len(ids_items_pendientes), tamanio_lote):
            ...
            res_f = (
                ...
                .select("id, item_orden_id, talla_polines, color_polines")
                .in_("item_orden_id", lote_items)
                .eq("polines_comprados", False)
                .execute()
            )
            if res_f.data:
                especs_data.extend(res_f.data)

            res_n = (
                supabase.table("especificaciones_producto")
                .select("id, item_orden_id, talla_polines, color_polines")
                .in_("item_orden_id", lote_items)
                .is_("polines_comprados", "null")
                .execute()
            )
            if res_n.data:
                especs_data.extend(res_n.data)
    ...
    df_join = pd.merge(
        df_esp,
        df_itm,
        left_on="item_orden_id",
        right_on="id",
        suffixes=("_esp", "_itm"),
    )
    df_join = pd.merge(
        df_join, df_ord, left_on="orden_id", right_on="id", suffixes=("", "_ord")
    )

    # Renombramos y limpiamos la vista
    df_join["color_polines"] = (
        df_join["color_polines"].fillna("SIN COLOR").astype(str).str.strip().str.upper()
    )
    df_join["color_polines"] = df_join["color_polines"].replace(
        {"": "SIN COLOR", "NONE": "SIN COLOR"}
    )
    ...
    ordenes_sel = col2.multiselect(
        "📦 Filtrar por Órdenes Específicas", lista_ordenes, default=lista_ordenes
    )
    ...
    st.caption(
        "Verifica quién solicitó cada color y corrige la columna 'Color a Comprar' para unificar pedidos."
    )

    # [Seguro]: Agrupamos órdenes, clientes y vendedoras involucradas por cada color escrito
    df_info_colores = (
        df_join.groupby("color_polines")
        .agg(
            {
                "codigo_orden": lambda x: " | ".join(x.unique()),
    ...
    df_map = pd.DataFrame(
        {
            "Orden(es)": df_info_colores["codigo_orden"],
    ...
    df_map_edit = st.data_editor(
        df_map,
        disabled=["Orden(es)", "Cliente(s)", "Color Original", "Vendedora(s)"],
    ...
    mapa_colores = dict(
        zip(
            df_map_edit["Color Original"],
            df_map_edit["Color a Comprar"].str.strip().str.upper(),
        )
    )
    ...
    df_resumen = (
        df_join.groupby(["talla_polines", "Color Consolidado"])
        .size()
        .reset_index(name="Cantidad a Comprar")
    )
    ...
    with col_tabla:
        ...
        resumen_tallas = (
            df_resumen.groupby("Talla")["Cantidad a Comprar"].sum().reset_index()
        )
    ...
    with col_accion:
        ...
        st.caption(
            f"Se marcarán **{len(df_join)}** pares de polines como 'Comprados' en la base de datos."
        )

        if st.button(
            "✅ Registrar Compra Realizada", type="primary", use_container_width=True
        ):
            try:
                with st.spinner("Actualizando base de datos..."):
                    ...
                    st.success(
                        "¡Compra registrada exitosamente! Los polines han sido retirados de la lista."
                    )
                    time.sleep(1.5)
                    st.rerun()
